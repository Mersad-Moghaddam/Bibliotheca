...
export function ProfilePage() {
  ...
  const profileIncomplete = !reminderForm.watch('enabled')
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <SectionCard>
        <SectionHeader title={t('journey.profileActionsTitle')} description={t('journey.profileActionsDescription')} />
        <div className="grid gap-3 md:grid-cols-2">
          <ContextActionCard
            title={t('journey.actions.completeReminder.title')}
            description={t('journey.actions.completeReminder.description')}
            actionLabel={t('journey.actions.completeReminder.cta')}
            icon={<Flame className="h-4 w-4" />}
            onAction={() => reminderForm.setValue('enabled', true)}
          />
          <ContextActionCard
            title={t('journey.actions.updateSecurity.title')}
            description={t('journey.actions.updateSecurity.description')}
            actionLabel={t('journey.actions.updateSecurity.cta')}
            icon={<Shield className="h-4 w-4" />}
            onAction={() => passwordForm.setFocus('currentPassword')}
          />
        </div>
      </SectionCard>
      ...
      <div className="grid gap-3 xl:grid-cols-2">
        ...
        <SectionCard>
          <SectionHeader title={t('profile.updatePassword')} icon={<Shield className="h-4 w-4" />} />
