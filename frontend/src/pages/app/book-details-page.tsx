  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <SectionCard>
        ...
        <div className="flex flex-wrap gap-2">{statusOptions.map((status) => <Button key={status} size="sm" variant="secondary" className="w-full sm:w-auto" onClick={() => updateStatus.mutate({ id: book.id, status })}>{t('books.moveTo')} {t(`status.${status}`)}</Button>)}</div>
